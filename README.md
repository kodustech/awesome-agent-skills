...
| [video-router](https://github.com/Orkas-AI/Orkas-VideoStudio/tree/main/packages/skills/video-router) | Routes plain-language video requests to OrkasVideoStudio's compose, edit, generate, or automatic pipelines, backed by an editable `plan.json` timeline plus CLI and MCP tools. MIT licensed; installs from source while npm publishing is pending. |
